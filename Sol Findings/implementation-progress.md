...
| 08 | [Synthetic-key ownership](08-synthetic-key-ownership.md) | Should fix | Verified | Report ownership now settles in the finalize hook on the final event result, so a consumed press claims nothing in the report; modifier ownership separates physical (key down) from report (QMK registered) counts; new physical-ownership integration suite covers shifted-symbol holds, Shift+Enter, default-processed overlap, and consumed-modifier teardown; full host, firmware, memory, and fresh stack gates pass |
...
| 10 | [Pointing backlog bounds](10-pointing-backlog-bounds.md) | Should fix | Implemented, verification incomplete | 4-tap/32-step bounds remain enforced; dominant-axis transitions now cancel inactive-axis debt at the transition, with X-to-Y and Y-to-X coverage; flashed timing/feel matrix remains pending |
...
| 15 | [RGB render work](15-rgb-render-work.md) | Optimize | Verified | Frame-local caching/work reductions remain verified; split worker publication now hands readers one coherent generation per domain, with deterministic seam-driven interleaving tests for the split, pd-mode, and RGB readers; full host, firmware, memory, and fresh stack gates pass |
