...
KEYMAP_C = SCRIPT_DIR.parent / "keyboards" / "bastardkb" / "charybdis" / "4x6" / "keymaps" / "noah" / "keymap.c"
...
# Must match the layer enum order in keymap.c.
...
REPLACEMENTS = {
    ...
    # keymap.c's custom_keycodes enum (0-indexed from SAFE_RANGE).
    ...
    # → Adding a new custom keycode?  Append it to the enum in keymap.c,
    ...
    "CUSTOM(83)": "DRG_TOG_ON_HOLD",  # enum position 19
}
...
def write_to_keymap():
    ...
    if not KEYMAP_C.exists():
        print(f"ERROR: keymap.c not found: {KEYMAP_C}", file=sys.stderr)
        sys.exit(1)
