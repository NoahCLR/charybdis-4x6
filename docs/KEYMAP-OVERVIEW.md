...
| State | Meaning | Authored HSV | Preview Color |
| --- | --- | --- | --- |
| `Tap Pending` | Neutral unresolved state for double-tap and higher gestures, shown only while the winning tap index is still open because a deeper authored tap branch is still reachable. The base single-tap candidate stays quiet, and reaching the deepest authored branch settles the index and goes straight to branch-confirm. | `HSV(0, 0, 150)` | <img alt="Tap Pending color" src="media/profile-introspection/profile-color-swatch-ffffff.svg" width="96" height="28" /> |
