...
| `tools/check_firmware_memory_budget.py` | Enforce compact macro symbols, static-BSS span, and linker-heap headroom against a fresh ordinary target ELF | read-only report; no source output | `run_firmware_memory_budget_tool_tests.sh`; fresh ordinary target build plus `run_firmware_memory_budget_checks.sh` |
