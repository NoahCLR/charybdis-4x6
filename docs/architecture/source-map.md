...
| `profile/runtime/` | Effective-profile publication and activation safety policy | Pending/active profile generation plus activation wait evidence | Validated profile snapshots, authoritative runtime activity, injected peer convergence | Coherent generation publication, ordered invalidation callbacks, reason/count status | `run_effective_profile_provider_tests.sh`, `run_profile_activation_policy_tests.sh` | active live-edit architecture review |
