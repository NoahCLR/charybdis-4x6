...
Freshly measured on 2026-08-27 after the first effective layer-RGB consumer:

| Measurement | Bytes | Meaning |
| --- | ---: | --- |
| SRAM0–3 `.data` | 22,984 | nonzero-initialized fixed data |
| SRAM0–3 `.bss` | 25,756 | zero-initialized fixed data |
| SRAM0–3 `.data + .bss` | 48,740 | regression metric, not total SRAM use |
| SRAM0–3 linker/core-memory span at boot | 213,400 | maximum allocator span before runtime allocations |
| Fixed linked section bytes across all SRAM banks | 56,200 | includes alignment, `.data`, `.bss`, RAM-resident sections, and reserved stacks; excludes runtime allocation |
...
| --- | ---: | ---: |
| SRAM0–3 `.bss` maximum | 26,000 B | 244 B |
| SRAM0–3 `.data + .bss` maximum | 51,000 B | 2,260 B |
| SRAM0–3 linker/core-memory span minimum at boot | 204,800 B | 8,600 B |
...
`.data + .bss` metric to about 52,836 bytes and leave about 209,304 bytes in the
boot core-memory span. Two would raise the metric to about 56,932 bytes and
leave about 205,208 bytes. Both designs violate current regression policies;
