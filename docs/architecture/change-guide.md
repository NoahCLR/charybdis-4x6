...
| --- | --- | --- | --- |
| Add or change a key behavior row | `keymap.c` `key_behaviors[]` | key-runtime reducer/planners unless semantics change | `profile_introspect.py --write`, `profile_introspect.py --check`, key behavior lookup/validation, keymap validation, real profile validation |
...
| Change RGB colors or LED groups | `rgb_config.c` | RGB runtime stage logic | profile introspection write/check, RGB validation, RGB render, real profile validation |
...
| Authored profile data | `profile_introspect.py --write`, `profile_introspect.py --check`, `run_key_behavior_lookup_tests.sh`, `run_key_behavior_validation_tests.sh`, `run_keymap_validation_tests.sh`, `run_real_profile_validation_tests.sh`, `run_real_profile_thumb_layer_lock_integration_tests.sh` |
