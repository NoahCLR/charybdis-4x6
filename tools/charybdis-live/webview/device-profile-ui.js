...
function renderDeviceProfileDetails(document, model, post = () => {}) {
    ...
    host.append(element("p", "These rows were read from the keyboard. A timing value of 0 uses the firmware default; the default duration is not reported. Saving behaviour changes is not supported yet.", "muted"));
    if (!rows.length) {
    ...
    for (const title of ["Key", "Timing (ms)", "Branches", "Auto-mouse anchor flag"]) heading.append(element("th", title));
    ...
    for (const row of rows) {
        ...
        key.append(element("code", row.keycode));
        ...
        for (const step of row.steps) {
            ...
            for (const [label, action] of [["Tap", step.tap], ["Hold", step.hold], ["Long hold", step.longHold]]) {
                if (!action) continue;
                const repeat = action.helper === "REPEAT_WHILE_HELD" ? `, ${action.repeatHz} Hz` : "";
                branch.append(element("div", `${label}: ${action.helper}(${action.action}${repeat})`));
            }
