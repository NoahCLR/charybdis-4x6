...
def render_key_behavior_feedback_section(profile: dict[str, object]) -> str:
    ...
    if not feedback_colors:
        lines.extend(
            [
                "No authored key-behavior feedback colors were found in `rgb_config.c`.",
                "",
            ]
        )
        return "\n".join(lines)
...
def render_generated_assets_section() -> str:
    return "\n".join(
        [
            ...
            f"- Generated color swatches: `./{ASSET_OUTPUT_DIR.name}/{COLOR_SWATCH_PREFIX}*.svg`",
            f"- Regenerate the report, layer images, and swatches with `python3 tools/profile_introspect.py --write`",
