...
JSON_OUTPUT = ASSET_OUTPUT_DIR / "profile-summary.json"
LAYER_IMAGE_PREFIX = "profile-layer-"
LEGACY_FLAT_JSON_OUTPUT = OUTPUT_DIR / "profile-summary.json"
...
def load_layout_template(keyboard_json_override: Path | None = None) -> list[dict[str, object]]:
...
def build_profile_model(keyboard_json_override: Path | None = None) -> dict[str, object]:
    ...
    layout_template = load_layout_template(keyboard_json_override)
...
def render_json_section() -> str:
    return "\n".join(
        [
            "## Diffable JSON",
            "",
            f"- Generated artifact: [`{JSON_OUTPUT.relative_to(REPO_ROOT)}`](./{ASSET_OUTPUT_DIR.name}/profile-summary.json)",
            f"- Generated layer images: `./{ASSET_OUTPUT_DIR.name}/{LAYER_IMAGE_PREFIX}*.svg`",
...
def managed_generated_artifact_paths(assets: dict[Path, str]) -> set[Path]:
    ...
    managed_paths.add(LEGACY_FLAT_JSON_OUTPUT)
