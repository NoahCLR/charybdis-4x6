...
static uint16_t key_runtime_slot_release_trace_flags(const key_runtime_slot_release_context_t *context, key_runtime_slot_release_phase_contract_t phase_contract,
    ...
    if (phase_contract.checks_fallback_hold_suppression && context->contract.fallback_hold_suppresses_nonquick_release &&
        !context->quick_tap && decision.outcome == KEY_RUNTIME_SLOT_RELEASE_DECISION_OUTCOME_NONE) {
...
static key_runtime_slot_release_query_t key_runtime_slot_release_query(const key_runtime_slot_release_context_t *context,
...
static key_runtime_slot_release_decision_t key_runtime_slot_resolve_release(active_key_state_t released_key, uint16_t elapsed) {
    ...
    key_runtime_trace_release_resolution(phase, key_runtime_slot_release_trace_outcome(decision.outcome), key_runtime_slot_release_trace_flags(&context, phase_contract, decision),
                                         key_runtime_slot_release_trace_detail(&context, decision));
