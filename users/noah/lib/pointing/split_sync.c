...
static bool             pd_sync_sent_once = false;

static void pd_sync_broadcast(const pd_sync_packet_t *pkt) {
    if (pd_sync_sent_once && memcmp(&pd_sync_last_sent, pkt, sizeof(pd_sync_packet_t)) == 0) {
        return;
    }
    if (transaction_rpc_send(PUT_PD_SYNC, sizeof(*pkt), pkt)) {
        pd_sync_last_sent = *pkt;
        pd_sync_sent_once = true;
    }
}
...
static void pd_sync_slave_rpc(uint8_t initiator2target_buffer_size, const void *initiator2target_buffer, uint8_t target2initiator_buffer_size, void *target2initiator_buffer) {
    ...
    pd_mode_apply_remote_snapshot(pd_sync_remote.pd_mode_flags, pd_sync_remote.pd_mode_locked_flags);
}
