...
enum {
    // One staged Raw HID chunk per validation step keeps validation bounded.
...
typedef struct {
    ...
    // Each step must return IN_PROGRESS, VALID, REJECTED, or IO_ERROR. OK is
    // deliberately not a terminal validation result and is treated as an
    // invalid backend response/storage failure by the owner.
...
// Scan side: processes at most one mailbox item or one bounded validation step.
