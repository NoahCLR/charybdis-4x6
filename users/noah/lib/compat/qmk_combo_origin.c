...
static bool combo_origin_combo_build_from_pressed_keys(uint16_t combo_index, uint8_t *out_bitmap, keypos_t *out_owner_key_pos) {
    ...
    for (uint16_t member_index = 0;; member_index++) {
        ...
        found_any = true;
        if (physical_key_states[key_origin_keypos_index(matched_key_pos)].press_sequence >= latest_press_sequence) {
            latest_press_sequence = physical_key_states[key_origin_keypos_index(matched_key_pos)].press_sequence;
            latest_key_pos        = matched_key_pos;
        }
    }
...
static bool combo_origin_collect_matching_active_combos(uint16_t keycode, uint16_t *primary_combo_index, keypos_t *primary_owner_key_pos, uint8_t *out_bitmap) {
    ...
    for (uint16_t combo_index = 0; combo_index < noah_combo_count; combo_index++) {
        ...
        if (!combo_origin_combo_build_from_pressed_keys(combo_index, combo_bitmap, &combo_owner_key_pos)) {
            continue;
        }
