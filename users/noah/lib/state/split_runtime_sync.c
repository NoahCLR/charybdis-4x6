...
static split_runtime_sync_packet_t split_runtime_sync_build_packet(uint16_t raw_elapsed) {
    bool    automouse_active = false;
    bool    automouse_render = false;
    uint8_t automouse_flags  = SPLIT_RUNTIME_SYNC_AUTOMOUSE_FLAG_NONE;

#    ifdef POINTING_DEVICE_AUTO_MOUSE_ENABLE
    automouse_active = is_auto_mouse_active();
#        if defined(RGB_AUTOMOUSE_GRADIENT_ENABLE)
    automouse_render = automouse_rgb_should_render_from_state(automouse_active, raw_elapsed);
#        endif
#    endif
    ...
    if (automouse_render) {
        automouse_flags |= SPLIT_RUNTIME_SYNC_AUTOMOUSE_FLAG_RENDER;
    }

    return (split_runtime_sync_packet_t){
#    if defined(POINTING_DEVICE_AUTO_MOUSE_ENABLE) && defined(RGB_AUTOMOUSE_GRADIENT_ENABLE)
        .automouse_progress = (!automouse_render || pd_any_mode_locked()) ? 0 : automouse_rgb_quantize_progress(raw_elapsed),
#    else
        .automouse_progress = 0,
#    endif
        .automouse_flags = automouse_flags,
#    ifdef POINTING_DEVICE_ENABLE
