...
static int8_t layer_ownership_find_slot_for_key(keypos_t key_pos) {
    for (uint8_t i = 0; i < ARRAY_SIZE(layer_momentary_bindings); i++) {
...
static int8_t layer_ownership_find_free_slot(void) {
    for (uint8_t i = 0; i < ARRAY_SIZE(layer_momentary_bindings); i++) {
...
static bool layer_ownership_remove_slot(uint8_t slot) {
...
void layer_ownership_momentary_press(keypos_t key_pos, uint8_t layer) {
    ...
    int8_t slot = layer_ownership_find_slot_for_key(key_pos);
    if (slot >= 0) {
        ...
        layer_ownership_remove_slot((uint8_t)slot);
    } else {
...
bool layer_ownership_momentary_release(keypos_t key_pos) {
    ...
    return layer_ownership_remove_slot((uint8_t)slot);
}
