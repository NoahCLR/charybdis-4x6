...
static bool rgb_runtime_key_feedback_stage_render_groups(const uint8_t *semantic_map, const uint8_t *tap_branch_map, const uint8_t *flash_visibility_bitmap, const uint8_t *broad_owner_map, uint8_t led_min, uint8_t led_max) {
    ...
    for (uint8_t pass = 0; pass < 2u; pass++) {
        const bool all_base_pass = pass == 0u;
        for (uint8_t group_index = 0; group_index < key_behavior_feedback_led_group_count; group_index++) {
            ...
            if ((group->semantic == KEY_FEEDBACK_GROUP_ALL) != all_base_pass) {
                continue;
            }
            painted |= rgb_runtime_key_feedback_stage_render_group(group, semantic_map, tap_branch_map, flash_visibility_bitmap, broad_owner_map, led_min, led_max);
        }
    }
...
static bool rgb_runtime_key_feedback_stage_render_impl(const uint8_t *semantic_map, const uint8_t *tap_branch_map, const uint8_t *flash_visibility_bitmap, const uint8_t *broad_owner_map, uint8_t led_min, uint8_t led_max) {
    bool painted = false;
...
bool rgb_runtime_key_feedback_stage_render(const uint8_t *semantic_map, const uint8_t *tap_branch_map, const uint8_t *flash_visibility_bitmap, const uint8_t *broad_owner_map, uint8_t led_min, uint8_t led_max) {
    return rgb_runtime_key_feedback_stage_render_impl(semantic_map, tap_branch_map, flash_visibility_bitmap, broad_owner_map, led_min, led_max);
}
