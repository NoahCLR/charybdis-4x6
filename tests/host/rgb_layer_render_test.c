...
#if RGB_LAYER_RENDER_TEST_FEEDBACK_GROUPS
static const key_behavior_feedback_led_group_t key_behavior_feedback_led_groups_data[] = RGB_LED_GROUP_TABLE({.semantic = KEY_FEEDBACK_GROUP_TAP_BRANCH_PENDING, .color = HSV(11, 12, 13), .led_group = RGB_LED_GROUP(5)}, {.semantic = KEY_FEEDBACK_GROUP_TAP_COMMITTED, .color = HSV(17, 18, 19), .led_group = RGB_LED_GROUP(5)}, {.semantic = KEY_FEEDBACK_GROUP_LONG_HOLD_ACTIVE, .color = HSV(20, 21, 22), .led_group = RGB_LED_GROUP(5)}, {.semantic = KEY_FEEDBACK_GROUP_ALL, .color = HSV(0, 0, 0), .led_group = RGB_LED_GROUP(5, 6)}, );
...
static void test_slave_remote_combo_overlay_suppresses_stale_pending_feedback(void) {
    ...
    layer_state    = (layer_state_t)1u << LAYER_SYM;
    test_feedback_bitmap_set(split_runtime_sync_remote.combo_overlay_bitmap, 0, 0);
    test_remote_feedback_semantic_add(0, 0, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
...
static void test_tap_branch_pending_uses_branch_color(void) {
    test_reset();

    layer_state = (layer_state_t)1u << LAYER_SYM;
    test_local_feedback_semantic_add(0, 0, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
...
static void test_tap_branch_pending_color_clamps_to_last_configured_color(void) {
    test_reset();
...
static void test_key_feedback_led_groups_repaint_after_feedback_locality(void) {
    ...
    check_led(0, rgb_from_tap_branch_color(0));
    check_led(5, rgb_from_key_feedback_group(0));
    check_led(6, rgb_from_feedback_semantic(KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING, 0u));
}
...
static void test_key_half_feedback_paints_only_master_half(void) {
    ...
    check_led(0, rgb_from_tap_branch_color(0));
    check_led(3, rgb_from_tap_branch_color(0));
...
static void test_key_half_feedback_paints_only_slave_half_from_remote_snapshot(void) {
    ...
    layer_state    = (1UL << LAYER_SYM);
    fake_is_master = false;
    test_remote_feedback_semantic_add(4, 0, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);

    CHECK(noah_rgb_matrix_indicators_advanced_user(0, RGB_MATRIX_LED_COUNT));

    check_led(0, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(3, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(4, rgb_from_tap_branch_color(0));
    check_led(7, rgb_from_tap_branch_color(0));
}
...
static void test_key_half_feedback_paints_both_halves_for_combo_footprint(void) {
    ...
    layer_state = (1UL << LAYER_SYM);
    test_local_feedback_semantic_add(0, 0, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
    test_local_feedback_semantic_add(4, 0, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
...
static void test_key_feedback_paints_only_master_key(void) {
    ...
    layer_state = (1UL << LAYER_SYM);
    test_local_feedback_semantic_add(0, 1, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);

    CHECK(noah_rgb_matrix_indicators_advanced_user(0, RGB_MATRIX_LED_COUNT));

    check_led(0, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(1, rgb_from_tap_branch_color(0));
    check_led(2, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(4, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(7, rgb_from_hsv(layer_colors[LAYER_SYM].color));
}
...
static void test_key_feedback_paints_only_slave_key_from_remote_snapshot(void) {
    ...
    layer_state    = (1UL << LAYER_SYM);
    fake_is_master = false;
    test_remote_feedback_semantic_add(4, 2, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
    ...
    check_led(4, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(5, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(6, rgb_from_tap_branch_color(0));
    check_led(7, rgb_from_hsv(layer_colors[LAYER_SYM].color));
}
...
static void test_key_feedback_paints_all_combo_keys_on_master(void) {
    ...
    layer_state = (1UL << LAYER_SYM);
    test_local_feedback_semantic_add(0, 0, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
    test_local_feedback_semantic_add(0, 2, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
    test_local_feedback_semantic_add(4, 1, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);

    CHECK(noah_rgb_matrix_indicators_advanced_user(0, RGB_MATRIX_LED_COUNT));

    check_led(0, rgb_from_tap_branch_color(0));
    check_led(1, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(2, rgb_from_tap_branch_color(0));
    check_led(4, rgb_from_hsv(layer_colors[LAYER_SYM].color));
    check_led(5, rgb_from_tap_branch_color(0));
    check_led(7, rgb_from_hsv(layer_colors[LAYER_SYM].color));
}
...
static void test_key_feedback_paints_all_remote_combo_keys_from_snapshot(void) {
    ...
    layer_state    = (1UL << LAYER_SYM);
    fake_is_master = false;
    test_remote_feedback_semantic_add(0, 1, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
    test_remote_feedback_semantic_add(4, 0, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
    test_remote_feedback_semantic_add(4, 2, KEY_FEEDBACK_SEMANTIC_TAP_BRANCH_PENDING);
...
int main(int argc, char **argv) {
    ...
    test_tap_branch_pending_uses_branch_color();
    test_tap_branch_pending_color_clamps_to_last_configured_color();
    test_slave_tap_branch_pending_uses_remote_branch_color();
#if RGB_LAYER_RENDER_TEST_FEEDBACK_GROUPS
