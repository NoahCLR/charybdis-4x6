...
#include "users/noah/lib/rgb/stages/rgb_combo_feedback_stage.h"
...
static const combo_feedback_led_group_t combo_feedback_led_groups_data[] = {
    {.color = HSV(201, 202, 203), .led_group = RGB_LED_GROUP(2)},
};
const combo_feedback_led_group_t *const combo_feedback_led_groups      = combo_feedback_led_groups_data;
const uint8_t                           combo_feedback_led_group_count = (uint8_t)ARRAY_SIZE(combo_feedback_led_groups_data);
...
static void test_compiled_and_live_combo_rendering(const char *fixture_path) {
    ...
    rgb_t                             compiled_color = hsv_to_rgb(combo_feedback_colors.color);
    rgb_t                             compiled_group = hsv_to_rgb(combo_feedback_led_groups[0].color);
    rgb_t                             live_color     = hsv_to_rgb((hsv_t){.h = 80u, .s = 81u, .v = 82u});

    key_origin_bitmap_fill_single(bitmap, (keypos_t){.row = 1u, .col = 0u});
    CHECK(rgb_runtime_combo_feedback_stage_render_effective_frame(&frame, NULL, bitmap, 0u, RGB_MATRIX_LED_COUNT));
    check_led(&frame, 2u, compiled_group);
    check_led(&frame, 6u, compiled_color);
    for (uint8_t led = 0u; led < RGB_MATRIX_LED_COUNT; led++) {
    ...
    CHECK(rgb_runtime_combo_feedback_stage_render_effective_frame(&frame, &profile_frame, bitmap, 0u, RGB_MATRIX_LED_COUNT));
...
int main(int argc, char **argv) {
    CHECK(argc == 2);
    test_compiled_and_live_combo_rendering(argv[1]);
    puts("rgb effective combo-feedback host tests passed");
    return 0;
}
