...
static unsigned backend_call_count(const fake_backend_t *fake) {
    return fake->begin_calls + fake->write_calls + fake->read_calls + fake->validation_begin_calls + fake->validation_step_calls + fake->abort_calls;
}
