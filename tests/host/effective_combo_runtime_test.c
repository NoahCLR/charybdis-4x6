...
bool is_combo_enabled(void) {return true;}
...
noah_profile_action_runtime_v1_result_t noah_profile_action_runtime_v1_to_native(const noah_profile_action_v1_t *action, uint16_t *native) {
    if (action->kind == 1u) *native = action->operand;
    else if (action->kind == 2u) *native = (uint16_t)(0x5220u + action->operand);
    else return NOAH_PROFILE_ACTION_RUNTIME_V1_UNSUPPORTED;
...
static uint32_t boundary(void *context) {return *(uint32_t *)context;}
static unsigned nibble(char value) {return value <= '9' ? (unsigned)(value - '0') : (unsigned)(value - 'a' + 10);}
int main(int argc, char **argv) {
    assert(argc == 2);
    FILE *fixture = fopen(argv[1], "r"); assert(fixture);
    char encoded[121]; assert(fscanf(fixture, "%120s", encoded) == 1); fclose(fixture);
    for (size_t index = 0; index < 60; index++) bytes[index] = (uint8_t)((nibble(encoded[2 * index]) << 4) | nibble(encoded[2 * index + 1]));
    ...
    for (unsigned index = 1; index < 32; index++) memcpy(&bytes[4 + 28 * index], &bytes[4], 28);
    ...
    view.profile.domain_mask = 4; view.profile.domain_count = 1; view.profile.byte_length = 60; view.profile.combos.row_count = 2;
