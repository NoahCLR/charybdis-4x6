...
| R-07 | Logical EEPROM growth or live-profile buffers consume an RP2040 SRAM bank without bank-aware accounting, or policy slack is mistaken for hardware headroom | high | 00 | **Occurred in the live-owner build**, measured on 2026-09-04: `.bss` 28,820 B against a 26,000 B policy limit and `.data`+`.bss` 51,820 B against 51,000 B, both failing, while roughly 205 KB of physical SRAM stays unused. Figures in `pointing-cadence-investigation.md`. Keep physical banks, linked sections, regression policies, and runtime high-water evidence separate; prefer repartitioning existing EEPROM; pass fresh memory and stack gates and measure hardware high-water before closure |
