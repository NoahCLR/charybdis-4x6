...
- users/noah/lib/key/behavior/key_behavior_lookup.c routes installed live
  generations through the effective behavior provider and retains a direct
  `key_behaviors[]` scan only as the compiled-default fallback.
- users/noah/lib/rgb/core/rgb_effective_config.c is now the sole compiled/live
  ...
  combo_feedback_colors/combo_feedback_led_groups plus key-feedback colors,
  group rows, and tap-commit policy; every current RGB render path consumes one
...
color, combo-feedback color/locality, key-feedback colors/policy/locality,
reusable group bitmaps, and every stage's group rows now consume that seam from
one renderer frame token. The runtime owner is installed only in the
side-specific engineering artifact; ordinary firmware still selects compiled
tables. Hot key events and RGB frames never replay the virtual compiled blob.
...
and SRAM5 banks. The 2026-08-30 ordinary ELF records 22,996 bytes of `.data`,
25,876 bytes of `.bss`, and therefore a 48,872-byte SRAM0–3 `.data + .bss`
regression metric. Its 51,000-byte ceiling has 2,128 bytes of policy slack;
that number is not total RAM headroom.

The current `__heap_base__` to `__heap_end__` span is 213,264 bytes. It is the
...
measured. Fixed linked occupancy across all banks is 56,336 bytes, including
...
queued and retryable rather than poisoning transaction state. The lease spans
validation, marker-last durability, and activation. The same backend can adopt
an exact boot-selected record through the bounded validator without rewriting
...
validation. Generic firmware deliberately exposes no durable origin. The
